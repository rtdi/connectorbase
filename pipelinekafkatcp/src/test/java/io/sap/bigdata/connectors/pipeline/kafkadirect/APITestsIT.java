...
import io.sap.bigdata.connectors.pipeline.kafkadirect.KafkaAPIdirect;
import io.sap.bigdata.connectors.pipeline.kafkadirect.KafkaConnectionProperties;
...
public class APITestsIT {
	...
	public static ValueSchema getValueSchema() throws SchemaException {
		...
		AvroRecordArray addressschema = valuebuilder.addColumnRecordArray("Addresses", null, true, "ADDRESS", null);
	...
	public void testSchemaCreate() {
		try {
			...
			SchemaHandler employeeschema = api.registerSchema(new SchemaName(tenantid, "Employee"), "The employee schema", keyschema, valueschema);
